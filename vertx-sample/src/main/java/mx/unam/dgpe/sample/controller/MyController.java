...
private void calcu(RoutingContext routingContext) {
	...
	int num2 = Integer.parseInt(request.getParam("num2"));
        ...
        String resp = calcula(tipo,num1,num2,request);
        response.setStatusCode(200).
        putHeader("content-type", "application/json; charset=utf-8").
        end(resp);
}
...
private int factorial(int n) {
	int resultado = 1;
	    ...
	    String x = Integer.toString(resultado);
	    longitud = x.length();
...
private void divide(RoutingContext routingContext) {
   ...
   private String cadena(String fuente){
	...
	return Json.encodePrettily(info);
     ...
     private String calcula(String tipo, int num1, int num2, HttpServerRequest request){
        int resultado = 0;
	 ...
	 info.put("Nodo ip corriendo", request.localAddress().host());
